...
import {
  getEventConfigResponseSchema,
  putEventConfigBodySchema
} from './eventConfigs.schemas.js';
...
function validateMetodoPagoBizum(patch: Json): void {
  // Esperamos patch.selectores.metodoPago: [{ nombre: string, requiereReceptor?: boolean }, ...]
  const selectores = isObject(patch.selectores) ? (patch.selectores as Record<string, unknown>) : null;
  ...
  for (const raw of metodoPago) {
    ...
    const nombre = String(raw.nombre ?? '').trim().toLowerCase();
    const requiere = raw.requiereReceptor;
    if (nombre === 'bizum' && requiere !== true) {
      const err = new Error('Para "bizum", el campo requiereReceptor debe ser true.');
      // tipado mínimo para Fastify error handler
...
export default fp(async (app) => {
  ...
  app.put('/:eventId', {
    schema: {
      ...
      body: putEventConfigBodySchema,
      response: { 204: { type: 'null' } }
    }
  }, async (req, reply) => {
    const { eventId } = req.params as { eventId: string };
    const patch = req.body as Json;
    // Validación de negocio: Bizum → requiereReceptor === true
    try {
