...
const EventsQueryParams = z.object({
	...
	after: z
		.string()
		.optional()
		.describe('Cursor para paginación (ID del último elemento de la página anterior)'),
	// Ordenación
	sortBy: z
	...
	name: z
		...
		.optional()
		.describe(
			'Filtrar por nombre del evento (búsqueda parcial, case-insensitive). Ejemplo: "Feria"',
		),
	date: z
		.string()
		.datetime()
		.optional()
		.describe(
			'Filtrar eventos desde esta fecha en adelante (>=). Formato ISO 8601. Ejemplo: "2025-06-15T12:00:00.000Z"',
		),
	createdAt: z
		.string()
		.datetime()
		.optional()
		.describe(
			'Filtrar eventos creados desde esta fecha en adelante (>=). Formato ISO 8601. Ejemplo: "2025-10-20T13:34:58.180Z"',
		),
});
	...
	app.get(
		'/',
		{
			schema: {
				...
				response: {
					200: EventPageResponse.describe(
						'Lista paginada de eventos con metadatos de paginación',
					),
					400: ValidationErrorResponse.describe(
						'Error de validación en los parámetros de consulta',
					),
					401: UnauthorizedResponse.describe(
	...
	app.post(
		'/',
		{
			schema: {
				...
				body: EventCreate,
				response: {
					201: EventCreatedResponse.describe(
						'Evento creado exitosamente con ID y timestamps generados',
					),
					400: ValidationErrorResponse.describe(
						'Error de validación en los datos enviados (campos faltantes o con formato incorrecto)',
					),
					401: UnauthorizedResponse.describe(
						'Token de autenticación inválido o faltante',
					),
					409: ConflictResponse.describe(
	...
	app.put(
		'/:id',
		{
			schema: {
				...
				response: {
					200: Event.describe(
						'Evento actualizado exitosamente con todos los nuevos valores',
					),
					400: ValidationErrorResponse.describe(
						'Error de validación en los datos enviados o ID inválido',
					),
					401: UnauthorizedResponse.describe(
						'Token de autenticación inválido o faltante',
					),
	...
	app.patch(
		'/:id',
		{
			schema: {
				...
				response: {
					200: Event.describe(
						'Evento actualizado exitosamente con los campos modificados',
					),
					400: ValidationErrorResponse.describe(
						'Error de validación en los datos enviados o ID inválido',
					),
			...
			schema: {
				...
				response: {
					204: NoContentResponse.describe(
						'Evento marcado como inactivo exitosamente (sin contenido en respuesta)',
					),
					400: ValidationErrorResponse.describe('ID inválido (formato incorrecto)'),
					401: UnauthorizedResponse.describe(
						'Token de autenticación inválido o faltante',
