...
export default fp(async (app) => {
  ...
  app.post('/', { schema: { summary: 'Create reserva (TBD)' } },
    async () => { throw new NotImplementedError(); });

  app.put('/:id', { schema: { summary: 'Update reserva (TBD)' } },
    async () => { throw new NotImplementedError(); });

  app.delete('/:id', { schema: { summary: 'Delete reserva (TBD)' } },
    async (_req, reply) => { return noContent(reply); });
});
