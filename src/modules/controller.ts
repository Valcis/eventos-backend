...
export function makeController<TDomain, TCreate = unknown, TUpdate = TCreate, TQuery extends Filter<Document> = Filter<Document>, >(
    collection: string,
    ...
    return {
        ...
        replace: async (req: FastifyRequest, reply: FastifyReply) => {
            ...
            const updated = await repo.updateById(db, id, req.body as TUpdate);
            if (!updated) {
