...
export const ErrorResponse = z.object({
	...
	code: z
		.string()
		.describe(
			'Código de error interno. Ejemplo: "VALIDATION_ERROR", "NOT_FOUND", "INTERNAL_ERROR"',
		),
	error: z
		.string()
		.describe(
			'Nombre del error HTTP. Ejemplo: "Bad Request", "Not Found", "Internal Server Error"',
		),
	message: z
		.string()
		.describe(
			'Mensaje descriptivo del error. Ejemplo: "El evento con ID 507f1f77bcf86cd799439011 no fue encontrado"',
		),
	details: z
		.array(
			z.object({
				path: z
					...
					.describe('Ruta del campo con error. Ejemplo: ["body", "name"]'),
				message: z
					.string()
					.describe('Mensaje de validación. Ejemplo: "El nombre es requerido"'),
			}),
		)
		.optional()
		.describe('Detalles de errores de validación (solo en errores 400)'),
});
...
export const ValidationErrorResponse = z.object({
	...
	message: z
		.string()
		.describe('Mensaje general. Ejemplo: "Error de validación en los datos enviados"'),
	details: z
		.array(
			z.object({
...
export const UnauthorizedResponse = z.object({
	...
	message: z
		.string()
		.describe('Mensaje descriptivo. Ejemplo: "Token de autenticación inválido o expirado"'),
});
...
export const ForbiddenResponse = z.object({
	...
	message: z
		.string()
		.describe('Mensaje descriptivo. Ejemplo: "No tienes permisos para realizar esta acción"'),
});
...
export const ConflictResponse = z.object({
	...
	message: z
		.string()
		.describe(
			'Mensaje descriptivo. Ejemplo: "Ya existe un evento con ese nombre en esa fecha"',
		),
});
...
export const InternalErrorResponse = z.object({
	...
	message: z
		.string()
		.describe(
			'Mensaje descriptivo. Ejemplo: "Ocurrió un error inesperado. Por favor contacte al soporte."',
		),
});
 ...
 */
export function createPagedResponse<T extends z.ZodTypeAny>(itemSchema: T, description: string) {
	return z.object({
 ...
 */
export const NoContentResponse = z
	.void()
	.describe('Sin contenido (operación exitosa sin cuerpo de respuesta)');

/**
 * Schema de respuesta de creación exitosa (201)
 * El cuerpo contiene el recurso creado
 */
export function createCreatedResponse<T extends z.ZodTypeAny>(itemSchema: T, description: string) {
