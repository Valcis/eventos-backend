...
export function makeCrud<T extends Document>(opts: {
    ...
    return {
        async list(db: Db, query: any, after: string | undefined, limit: number): Promise<CursorPage<T>> {
        ...
        async create(db: Db, input: any): Promise<T> {
            ...
            const doc = toDb({...input, createdAt: now, updatedAt: now, isActive: input.isActive ?? true});
        ...
        async softDelete(db: Db, id: string): Promise<void> {
            ...
            const _id = new ObjectId(id);
            if (softDelete) {
                await col.updateOne({_id}, {$set: {isActive: false, updatedAt: new Date().toISOString()}});
            } else {
