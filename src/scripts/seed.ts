...
async function run(): Promise<void> {
    ...
    await db.collection('precios').insertMany([
        {eventId, concepto: 'Parrillada', importe: 12.5, moneda: 'EUR', isActive: true, createdAt: now, updatedAt: now},
        {eventId, concepto: 'Picarones', importe: 4.0, moneda: 'EUR', isActive: true, createdAt: now, updatedAt: now}
    ]);

    // 3) gastos
    await db.collection('gastos').insertMany([
        {
            eventId, producto: 'Carbón 5kg', cantidad: 2,
            tipoPrecio: 'con IVA', tipoIVA: 21, precioBase: 10, precioNeto: 12.1,
            comprobado: false, locked: false, isActive: true,
            pagadorId: 'c1', tiendaId: 'p1',
    ...
    await db.collection('reservas').insertMany([
        {
            eventId, cliente: 'Juan Pérez', parrilladas: 1, picarones: 0,
            metodoPagoId: 'bz1', receptorId: 'rc1', tipoConsumoId: 'lc1', comercialId: 'c1',
            totalPedido: 24.0, pagado: true, comprobado: false, locked: false,
            puntoRecogidaId: 'p1', isActive: true, createdAt: now, updatedAt: now
        }
    ]);
