...
export async function ensureMongoArtifacts(db: Db): Promise<void> {
    ...
    await ensure(db, 'reservations', [
        ...
        { key: { eventId: 1, isPaid: 1, createdAt: -1 }, name: 'idx_reservations_eventId_isPaid_createdAt' },
        { key: { eventId: 1, isDelivered: 1, createdAt: -1 }, name: 'idx_reservations_eventId_isDelivered_createdAt' },
        { key: { salespersonId: 1, createdAt: -1 }, name: 'idx_reservations_salespersonId_createdAt' },
        { key: { paymentMethodId: 1, createdAt: -1 }, name: 'idx_reservations_paymentMethodId_createdAt' },
        { key: { cashierId: 1, createdAt: -1 }, name: 'idx_reservations_cashierId_createdAt' },
        { key: { eventId: 1, reserver: 1 }, name: 'uniq_reservations_eventId_reserver', unique: true, collation: ci as any },
    ]);

    // PRODUCTS
    await ensure(db, 'products', [
        { key: { eventId: 1, name: 1 }, name: 'uniq_products_eventId_name', unique: true, collation: ci as any },
        { key: { eventId: 1, categoryId: 1, name: 1 }, name: 'idx_products_eventId_categoryId_name' },
    ...
    const catalogs = [
        'units','salespeople','paymentmethods','cashiers','stores',
        'consumptiontypes','payers','pickuppoints','partners'
    ];
    for (const col of catalogs) {
        await ensure(db, col, [
            { key: { eventId: 1 }, name: `idx_${col}_eventId` },
            { key: { eventId: 1, name: 1 }, name: `uniq_${col}_eventId_name`, unique: true, collation: ci as any },
        ]);
    }
}
